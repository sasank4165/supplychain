...
Specialized agent for supplier analysis tasks for Procurement Specialists.
Invokes Supplier Analyzer Lambda function and integrates with Bedrock for tool calling.
"""
...
class SupplierAgent(BaseAgent):
    """
    Supplier Agent for Procurement Specialists.
    
    Provides supplier analysis capabilities including:
    ...
    TOOL_DEFINITIONS = [
        {
            "toolSpec": {
                ...
                "description": "Analyze supplier performance metrics including fill rate, on-time delivery rate, and quality metrics over a specified time period.",
                "inputSchema": {
                    "json": {
                        "type": "object",
                        "properties": {
                            "supplier_code": {
                                "type": "string",
                                "description": "The supplier code to analyze performance for"
                            },
                            "days": {
                                "type": "integer",
                                "description": "Number of days to look back for performance analysis"
                            }
                        },
                        "required": ["supplier_code", "days"]
                    }
                }
            }
        },
        {
            "toolSpec": {
                ...
                "description": "Compare costs across multiple suppliers for a specific product group. Returns cost comparison and recommendations.",
                "inputSchema": {
                    "json": {
                        "type": "object",
                        "properties": {
                            "product_group": {
                                "type": "string",
                                "description": "The product group to compare costs for"
                            },
                            "suppliers": {
                                "type": "array",
                                "items": {"type": "string"},
                        ...
                        "required": ["product_group", "suppliers"]
                    }
                }
            }
        },
        {
            "toolSpec": {
                ...
                "description": "Identify cost savings opportunities by analyzing price variations, volume discounts, and alternative suppliers. Returns actionable savings recommendations.",
                "inputSchema": {
                    "json": {
                        "type": "object",
                        "properties": {
                            "threshold_percent": {
                                "type": "number",
                                "description": "Minimum cost savings threshold as a percentage (e.g., 10.0 for 10% savings)"
                            }
                        },
        ...
        {
            "toolSpec": {
                "name": "analyze_purchase_trends",
                "description": "Analyze purchase order trends over time including volume, spending, and supplier distribution. Supports grouping by various dimensions.",
                "inputSchema": {
                    "json": {
                        "type": "object",
                        "properties": {
                            ...
                            "group_by": {
                                "type": "string",
                                "description": "Dimension to group by: 'supplier', 'product_group', or 'month'",
                                "enum": ["supplier", "product_group", "month"]
                            }
                        },
                        "required": ["days", "group_by"]
                    }
                }
            }
        }
    ...
    def __init__(
        ...
        Args:
            ...
            lambda_function_name: Name of the Supplier Analyzer Lambda function
            logger: Optional logger instance
        """
        super().__init__(agent_name="SupplierAgent", logger=logger)
    ...
    def process_request(
        ...
        """
        Process a supplier analysis request.
        ...
        try:
            ...
            # System prompt for supplier analysis
            ...
            return self.handle_exception(
                exception=e,
                context="processing supplier analysis request",
                execution_time=execution_time
            )
    ...
    def _get_system_prompt(self) -> str:
        """Get system prompt for supplier analysis."""
        return """You are a Supplier Analysis Assistant for Procurement Specialists.

Your role is to help procurement specialists analyze supplier performance, compare costs, identify savings opportunities, and make data-driven sourcing decisions.

You have access to the following tools:
- analyze_supplier_performance: Analyze supplier metrics (fill rate, on-time delivery, quality)
- compare_supplier_costs: Compare costs across multiple suppliers
